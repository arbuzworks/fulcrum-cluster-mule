 ...
 */
public class FulcrumEventCorrelator implements Startable, Stoppable, Disposable
{

    /**
     * logger used by this class
     */
    protected transient final Log logger = LogFactory.getLog(FulcrumEventCorrelator.class);
    ...
    private FulcrumEventCorrelator.ExpiringGroupMonitoringThread expiringGroupMonitoringThread;
    private final String name;

    private final boolean persistentStores;
    private final String storePrefix;

    public FulcrumEventCorrelator(EventCorrelatorCallback callback,
        ...
        while (true)
        {
            ...
            synchronized (groupsLock)
            {
                ...
                if (callback.shouldAggregateEvents(group))
                {
                    ...
                    if (VoidMuleEvent.getInstance().equals(returnEvent)) {
                        return null;
                    }
    ...
    private final class ExpiringGroupMonitoringThread extends EventProcessingThread implements Expirable, Disposable
    {
        ...
        public void doRun()
        {
            ...
            try
            {
                ...
                {
                    //FIXME start group can be null
                    EventGroup group = null;
                    try {
                        group = getEventGroup(o);
                    } catch (ObjectStoreException e) {
                        //ignored
                    }
                    if (group == null) {
